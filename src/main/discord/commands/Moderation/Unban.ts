...
async function run (interaction: Discord.ChatInputCommandInteraction): Promise<void> {
    return new Promise(async (resolve, reject) => {
        ...
        if (guildData.actionChannel) {
            const activeTimeouts = await sql.query(`SELECT * FROM ModActions WHERE guildID = '${interaction.guild.id}' AND userID = '${targetUser.id}' AND action = 'BAN' AND (status = 'ACTIVE' OR status = 'APPEALED' OR status = 'DENIED')`).catch((error) => {
        ...
        await sql.query(`UPDATE ModActions SET status = 'REVOKED' WHERE guildID = '${interaction.guild.id}' AND userID = '${targetUser.id}' AND action = 'BAN' AND (status = 'ACTIVE' OR status = 'APPEALED' OR status = 'DENIED')`).catch((error) => {
